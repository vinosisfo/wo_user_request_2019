...
class M_laporan_permintaan extends CI_Model {	
  ...
  public function data_status()
   {
      $query = $this->db->from('tb_permintaan')
                        ->group_by('status')
                        ->get();
      return $query;
   }
   ...
   public function data_gen_laporan($id_departemen,$t_awal,$t_akhir,$status){
    ...
    if(empty($status)){
       $query=$this->db->select('a.id_permintaan,b.nama_inventaris,a.tgl_permintaan,a.status,a.tgl_selesai,c.nama_departemen')
                         ...
                         ->where('a.tgl_permintaan >=', $t_awal)
                         ->where('a.tgl_permintaan <=', $t_akhir)
                         ->get();
      }
      if(!empty($status))
      {
   ...
   public function lap_detail($id_permintaan)
   {
    $query = $this->db->select('d.nama_karyawan,a.tgl_permintaan,a.tgl_app_manager_dept,a.tgl_selesai,e.nama_departemen,b.nama_inventaris,a.ket_permintaan,a.ket_tujuan,a.status,a.progres,a.gambar,a.id_permintaan,f.nama_teknisi,b.id_inventaris')
                         ->from('tb_permintaan a')
          ...
          if ($query->num_rows() > 0){
            foreach ($query->result() as $setuju) {
              ...
              $data_setuju=$this->db->select('a.nama_karyawan')
                                  ...
                                  ->join('tb_permintaan c','c.app_manager_dept=b.id_user')
                                  ->where('c.id_permintaan',$setuju->id_permintaan)
