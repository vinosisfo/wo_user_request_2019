...
<table class="customers">
    ...
    <tr>
        <td>created By @supriyono || 081317555430 || vinosisfo94@gmail.com</td>
    </tr>
</table>
<hr>
    <p>Laporan Permintaan Bulanan <?php echo date("F Y",strtotime($date1)).' s/d '.date("F Y",strtotime($date2)) ?></p>
<hr>
<table style="max-width: 98%; min-width : 95%;">
    <thead>
        <tr>
            <th style="text-align : left;">No</th>
            <th style="text-align : left;">Minggu</th>
            <th style="text-align : left;">Inventaris</th>
            <th style="text-align : left;">Masalah</th>
            <th style="text-align : left;">Penanganan</th>
            <th style="text-align : left;">Keterangan Teknisi</th>
            <th style="text-align : left;">Progres</th>
    ...
    foreach ($list->result() as $data) {
        $no++;
        $urut+=1;
        $type1=$data->nama_departemen.$data->BULAN.$data->TAHUN;
        if($type1<>$type2){
            if($urut>1){?>
            <tr>
                <td colspan="9">&nbsp;</td>
            ...
            <?php } ?>
                <tr>
                    <td colspan="9">
                        <?php 
                        $bulan = $data->TAHUN.'-'.$data->BULAN.'-01';
                        echo strtoupper($data->nama_departemen) ?> Bulan  : <?php echo date("F Y",strtotime($bulan)) ?><br>
                        <hr>
                    </td>
                </tr>
        <?php 
