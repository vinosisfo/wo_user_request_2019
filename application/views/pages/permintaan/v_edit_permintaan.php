    ...
    <div class="table-responsive">
        ...
        <table class="customers_a">
            <tr>
                ...
                <td>
                    <input type="hidden" name="id_permintaan" id="id_permintaan" readonly value="<?php echo $head->id_permintaan ?>">
                    <input type="text" name="tanggal" id="tanggal" class="tanggal" readonly style="width: 80px;" value="<?php echo $head->tgl_permintaan ?>">
                </td>
            </tr>
            <tr>
                <td>Departemen</td>
        ...
        <table class="table table-condensed table-bordered">
            ...
            <tbody id="row_body">
                ...
                foreach ($list->result() as $data) {
                    ...
                    <tr>
                        <td valign="top">
                            <img src="<?php echo base_url()?>assets/img/hapus.png" onclick="hapus_baris('<?php echo $data->id_permintaan?>','<?php echo $data->id_dtpermintaan ?>')" style="width:15px;" title="Hapus Data">
                        </td>
                        <td valign="top">
                            <input type="hidden" name="id_detail[]" id="id_detail_<?php echo $no ?>" value="<?php echo $data->id_dtpermintaan ?>" readonly\>
                        ...
                        <td valign="top">
                            <textarea name="keterangan[]" id="keterangan_<?php echo $no ?>" maxlength="200" style="resize:none;width : 250px; height : 40px;"><?php echo $data->keterangan ?></textarea>
                        </td>
                        <td valign="top">
                        <textarea name="tujuan[]" id="tujuan_<?php echo $no ?>" maxlength="200" style="resize:none;width : 250px; height : 40px;"><?php echo $data->tujuan ?></textarea>
                        </td>
                        <td valign="top">
                            ...
                            <img id="preview_<?php echo $no ?>'" src="<?php echo base_url()?>assets/img/permintaan/<?php echo $data->gambar ?>" alt="" style="width: 20%; height:20%;" onerror="this.onerror=null;this.src=\''+<?php echo base_url('assets/img/not_found.png')?>'\';"/>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
...
<script>
    function hapus_baris(id_permintaan,id_detail){
        if(confirm("Yakin Akan DiHapus?")){
            level = '<?php echo $level ?>'
            $.post("<?php echo base_url('c_permintaan_new/hapus_baris')?>",{id_permintaan : id_permintaan, id_detail : id_detail},function(data){
                if(data.hasil=="ok"){
                    alert("Data Berhasil Dihapus");
                    edit_data(id_permintaan,level)
                }
            },"json")
        }
    }
    // function simpan_data(){
        ...
        $.ajax({
            url        : '<?php echo base_url('c_permintaan_new/update_data') ?>',
            type       : "POST",
