            ...
            <div class="col-xs-12">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        <li class="active"><a href="#fa-icons" data-toggle="tab">Tampilkan Data pemrintaan</a></li>
                        <?php if($this->session->userdata['level']==='ADMIN'){?>
                        <li><button class="btn btn-danger" type="button" onclick="input_data(this)">Buat Permintaan</button></li>
                        <?php } ?>
                    </ul>
...
<script>
    function input_data(){
        $.post("<?php echo base_url('c_permintaan_new/input_data')?>",{format : "input_data"}, function(data){
            $(".judul").html("Buat Permintaan Baru")
    ...
    function edit_data(id_permintaan,level){
        $.post("<?php echo base_url('c_permintaan_new/edit_data')?>",{id_permintaan : id_permintaan,level : level}, function(data){
            $(".judul").html("Edit Permintaan Baru")
            $("#modal_form").modal("show")
            $("#form_data").html(data)
        })
    }

    function detail_data(id_permintaan,level){
        $.post("<?php echo base_url('c_permintaan_new/detail_data')?>",{id_permintaan : id_permintaan,level : level}, function(data){
            $(".judul").html("Detail Permintaan")
            $("#modal_form").modal("show")
            $("#form_data").html(data)
        })
    }

    function hapus_data(id_permintaan){
        if(confirm("Yakin Data Akan Dihapus")){
            $.post("<?php echo base_url('c_permintaan_new/hapus_data')?>",{id_permintaan : id_permintaan}, function(data){
                if(data.hasil=="ok"){
                    table.ajax.reload();
    ...
    function approve_data(id_permintaan,level){
        $.post("<?php echo base_url('c_permintaan_new/approve_data')?>",{id_permintaan : id_permintaan,level : level}, function(data){
            if(level=="TEKNISI"){
                $(".judul").html("Progres Permintaan "+level)
            } else {
                $(".judul").html("Approve Permintaan "+level)
            }
    ...
    table = $('#data_list').DataTable({ 
        ...
        "order"       : [],
        "ajax"        : {
        "url" : "<?php echo base_url('c_permintaan_new/get_data')?>",
        "type": "POST",
        "data": function ( data ) {
